...
def DrawSpatial_Model(shapefile,OutputFile_P,data,j,color):
    ...
    for i in range(len(Threshold)):
        for m in range(len(data_Variables)):
            ...
            # common_levels,norm=getColor(j,1,26,color)
            # common_levels,norm=getColor(j,1,31,color)
            # common_levels,norm=getColor(j,1,113,color)
            # common_levels,norm=getColor(j,67,202,color)
            common_levels,norm=getColor(j,133,328,color)
            for ti in time_period:
