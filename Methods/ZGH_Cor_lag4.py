...
def Read_Csv_Defomation_ve():
    csv_file="D:\zhaozheng\projects\全球风险计算\SBAS_INSAR\ZGH\zgh_2024\Rockfall.csv"
    Deformation_data=pd.read_csv(csv_file)
    Deformation_data['Date']=pd.to_datetime(Deformation_data['Date'],format='%Y/%m/%d')
    delate_days_array=[0]
    Deformation_Vectory_array=np.zeros([49,1])
    for t in range(Deformation_data['Date'].shape[0]-1):
        ...
        Deformation_Vectory_array=np.concatenate((Deformation_Vectory_array,Deformation_Vectory.reshape(49,1)),axis=1)
    ...
    Deformation_data.to_csv('D:\zhaozheng\projects\全球风险计算\SBAS_INSAR\ZGH\zgh_2024\Rockfall_All.csv',index=False)
    return Deformation_data

def calculate_Pre_TemBy_shape_average(lag,land_num,time_range_start,time_range_End):
    shp_file ="D:\zhaozheng\projects\全球风险计算\SBAS_INSAR\ZGH\zgh_2024\RF\RF.shp"
    ...
    for ct in range(len(Cal_Type)):
        ...
        np.savetxt(Output_Path+F"RF_{Cal_Type[ct]}_max{lag}.csv", array_type, delimiter=',')
