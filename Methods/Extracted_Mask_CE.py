...
FileNameList_Pr="F:\Experiments\Data_Processing\CMIP6_0.15_Binear\Pr\\ACCESS-CM2\\Historical\\*.nc"
FileNameList2_Pr="F:\Experiments\Data_Processing\CMIP6_0.15_Binear\Pr\\ACCESS-CM2\\126\\*.nc"
FileNameList3_Pr="F:\Experiments\Data_Processing\CMIP6_0.15_Binear\Pr\\ACCESS-CM2\\245\\*.nc"
FileNameList4_Pr="F:\Experiments\Data_Processing\CMIP6_0.15_Binear\Pr\\ACCESS-CM2\\585\\*.nc"

FileNameList_Tem="F:\Experiments\Data_Processing\CMIP6_0.15_Binear\Tem\\ACCESS-CM2\\Historical\\*.nc"
FileNameList2_Tem="F:\Experiments\Data_Processing\CMIP6_0.15_Binear\Tem\\ACCESS-CM2\\126\\*.nc"
FileNameList3_Tem="F:\Experiments\Data_Processing\CMIP6_0.15_Binear\Tem\\ACCESS-CM2\\245\\*.nc"
FileNameList4_Tem="F:\Experiments\Data_Processing\CMIP6_0.15_Binear\Tem\\ACCESS-CM2\\585\\*.nc"
...
Pr_theshold= xr.open_dataset("F:\Experiments\Data_Processing\Results\ACCESS-CM2_PR_1\\Historical_Threshold_Pr.nc").Threshold
Tem_theshold = xr.open_dataset("F:\Experiments\Data_Processing\Results\ACCESS-CM2_Tem\\Historical_Threshold_Tem").Threshold
...
for i in range(len(Module)):
    ...
    CE_Type=["CES_ACCESS-CM2","CET_ACCESS-CM2","CETS_ACCESS-CM2"]
    for j in range(len(CE_Type)):
        ...
        for key, group in mask_A.groupby(mask_A.time.dt.year):
            CE_Mask_P = group.rename('CE_Mask').to_netcdf(F"F:\Experiments\Data_Processing\Results\{CE_Type[j]}\\CE_Mask_{Module[i]}_{key}.nc", compute=False)
            with ProgressBar():
                CE_Mask_P.compute()

        File_Mask = open_mfdataset(F"F:\Experiments\Data_Processing\Results\{CE_Type[j]}\\CE_Mask_{Module[i]}_*.nc",
                                   parallel=True, engine='h5netcdf', concat_dim="time",
                                   chunks={'time': 20}, combine="nested", data_vars='minimal', coords='minimal',
                                   compat='override')
        ...
        CE_Duration_P = bbb.rename('CE_Duration').to_netcdf(F"F:\Experiments\Data_Processing\Results\{CE_Type[j]}\\CE_Duration_{Module[i]}.h5", compute=False)
        with ProgressBar():
            CE_Duration_P.compute()

        ccc=CE_Frequency(mask)
        CE_Frequency_P = ccc.rename('CE_Frequency').to_netcdf(f"F:\Experiments\Data_Processing\Results\{CE_Type[j]}\\CE_Frequency_{Module[i]}.h5", compute=False)
        with ProgressBar():
            CE_Frequency_P.compute()

        ddd=CE_Amplitude(mask)
        CE_Amplitude_P = ddd.rename('CE_Amplitude').to_netcdf(F"F:\Experiments\Data_Processing\Results\{CE_Type[j]}\\CE_Amplitude_{Module[i]}.h5", compute=False)
        with ProgressBar():
            CE_Amplitude_P.compute()

        ggg = CE_Return(mask)
        CE_Return_P = ggg.rename('CE_Return').to_netcdf(
            F"F:\Experiments\Data_Processing\Results\{CE_Type[j]}\\CE_Return_{Module[i]}.h5", compute=False)
        with ProgressBar():
            CE_Return_P.compute()

        hhh = CE_Start(mask)
        CE_Start_P = hhh.rename('CE_Start').to_netcdf(
            F"F:\Experiments\Data_Processing\Results\{CE_Type[j]}\\CE_Start_{Module[i]}.h5", compute=False)
        with ProgressBar():
            CE_Start_P.compute()

        iii = CE_End(mask)
        CE_End_P = iii.rename('CE_End').to_netcdf(
            F"F:\Experiments\Data_Processing\Results\{CE_Type[j]}\\CE_End_{Module[i]}.h5", compute=False)
        with ProgressBar():
            CE_End_P.compute()

        File_Mask.close()

        folder_path = F"F:\Experiments\Data_Processing\Results\{CE_Type[j]}"
