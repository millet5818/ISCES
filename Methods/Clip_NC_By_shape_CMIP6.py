...
def Read_Csv_Defomation_ve():
    csv_file="D:\zhaozheng\projects\全球风险计算\SBAS_INSAR\ZZ\Deformation\Deformation.csv"
    Deformation_data=pd.read_csv(csv_file)
    Deformation_data['Times']=pd.to_datetime(Deformation_data['Times'],format='%Y-%m-%d')
    delate_days_array=[0]
    Deformation_Vectory_array=np.zeros([73,1])
    for t in range(Deformation_data['Times'].shape[0]-1):
        delate_days=pd.Timedelta(Deformation_data['Times'][t+1] - Deformation_data['Times'][t]).days
        ...
        Deformation_Vectory_array=np.concatenate((Deformation_Vectory_array,Deformation_Vectory.reshape(73,1)),axis=1)
    ...
    Deformation_data.to_csv("D:\zhaozheng\projects\全球风险计算\SBAS_INSAR\ZZ\Deformation\Deformation_all.csv",index=False)
    return Deformation_data


def calculate_CEindicatorBy_shape_average():
        ...
        for v in range(len(Varibale)):
            ...
            for t in range(len(CE_Type)):
                ...
                data_clip = dataset[f'{CE_Type[t]}_{Varibale[v]}'].sel(
                    ...
                    lat=[aoi_lat[0], aoi_lat[1]],method="nearest").mean(dim=['lat','lon']).sel(time=Year,quantile=0.7)
                data_clip_p=data_clip.to_pandas()
                data_df[CE_Type[t]]=data_clip_p.values
            data_df.to_csv(Output_Path+F"{Nums[i]}_{Varibale[v]}.csv",index=False)
...
def calculate_CE_Indicator_byshape_average():
    ...
    for v in range(len(Varibale)):
        ...
        for t in range(len(CE_Type)):
            print([CE_Type[t]])
            filename=input_Path+CE_Type[t]+f"\\CMIP6\\{CE_Type[t]}_{Varibale[v]}_126.nc"
            dataset1 = xr.open_dataset(filename)
            base_filename=input_Path+CE_Type[t]+f"\\CMIP6\\{CE_Type[t]}_{Varibale[v]}_Historical.nc"
            base_file=xr.open_dataset(base_filename)
            dataset=xr.concat((base_file,dataset1),dim='time')
            # slice适用于至少数据大于一个分辨率的情况，适用于大区域
            data_clip = dataset[f'{CE_Type[t]}_{Varibale[v]}'].sel(
...
def calculate_ExtrePreindicatorBy_shape_average():
    shp_file ="D:\zhaozheng\projects\全球风险计算\SBAS_INSAR\ZZ\Landslides\Landslides.shp"
    Output_Path="E:\CE_DATA\Data_Processing\Average_Mode\Cut_ZZ\Typical Landslides\\"
    input_Path="E:\CE_DATA\Data_Processing\Average_Mode\\"
    shapefile = gpd.read_file(shp_file)
    ...
    for i in range(len(Nums)):
        ...
        for t in range(len(CE_Type)):
                ...
                dataset = xr.open_dataset(filename)
                # slice适用于至少数据大于一个分辨率的情况，适用于大区域
                data_clip = dataset[{CE_Type[t]}].sel(
                    lon=[aoi_lon[0], aoi_lon[1]],
                    lat=[aoi_lat[0], aoi_lat[1]],method="nearest").mean(dim=['lat','lon']).sel(time=Year,quantile=0.7)
                data_clip_p=data_clip.to_pandas()
                data_clip_p.to_csv(Output_Path+F"{Nums[i]}_{CE_Type[t]}.csv")
...
def calculate_ExtreTtemindicatorBy_shape_average():
    shp_file ="D:\zhaozheng\projects\全球风险计算\SBAS_INSAR\ZZ\Landslides\Landslides.shp"
    Output_Path="E:\CE_DATA\Data_Processing\Average_Mode\Cut_ZZ\Typical Landslides\\"
    input_Path="E:\CE_DATA\Data_Processing\Average_Mode\\"
    shapefile = gpd.read_file(shp_file)
    Year=np.arange(2015,2023,1).astype(str)
    Nums=np.array([58,57,53,52,5,6,4,3,30,1,2,41,42,84,90,16,89,92,29])
    CE_Type=['T95Max_L','T95D']
    Varibale_Units=['T95Max_L','T90D']
    for i in range(len(Nums)):
        print(Nums[i])
        Part_Shape=shapefile[shapefile.OBJECTID==Nums[i]]# 获取部分要素
        Boundary=Part_Shape.total_bounds
        ...
        for t in range(len(CE_Type)):
                ...
                dataset = xr.open_dataset(filename)
                # slice适用于至少数据大于一个分辨率的情况，适用于大区域
                data_clip = dataset[{Varibale_Units[t]}].sel(
                    longitude=[aoi_lon[0], aoi_lon[1]],
                    latitude=[aoi_lat[0], aoi_lat[1]],method="nearest").mean(dim=['latitude','longitude']).sel(time=Year,quantile=0.7)
                data_clip_p=data_clip.to_pandas()
                data_clip_p.to_csv(Output_Path+F"{Nums[i]}_{CE_Type[t]}.csv")

# calculate_ExtreTtemindicatorBy_shape_average()
...
def calculate_Pre_TemBy_shape_average3(time_range,function):
    ...
    for ct in range(len(Cal_Type)):
        ...
        for i in range(len(Nums)):
            ...
            data_df[f'{str(Nums[i])}_{Cal_Type[ct]}']=time_Data
    # data_df.to_csv(f'{Output_Path}Pre_Tem_{function}.csv',index=False)
    data_df.to_csv(f'{Output_Path}Tem_{function}.csv',index=False)





def calculate_Pre_TemBy_land_average():
    shp_file ="D:\zhaozheng\projects\GAT_LSM\shp\L4.shp"
    Output_Path="E:\CE_DATA\Data_Processing\Average_Mode\Cut_ZZ\Typical Landslides\\Daily\\"
    input_Path="E:\CE_DATA\Data_Processing\Average_Mode\\"
    shapefile = gpd.read_file(shp_file)
    Nums=np.array(['L4'])
    for i in range(len(Nums)):
        Part_Shape=shapefile[shapefile.Name==Nums[i]]# 获取部分要素
        Boundary=Part_Shape.total_bounds
        aoi_lat = [float(Boundary[1]), float(Boundary[3])]
        aoi_lon = [float(Boundary[0]), float(Boundary[2])]
        Cal_Type=['Daily_Pre','Daily_Tem']
        ...
        Variable=['pr','tasmax']
        for ct in range(len(Cal_Type)):
            filename= input_Path+f"{Cal_Type[ct]}\\CMIP6\\{Filename[ct]}_2020.nc"
            ...
            data_clip.to_pandas().to_csv(f'{Output_Path}_{Nums[i]}{Cal_Type[ct]}.csv')

calculate_Pre_TemBy_land_average()
...
# time_pairs_lag= [(deformation['Times'][i] - pd.Timedelta(days=12), deformation['Times'][i]) for i in range(len(deformation['Times'])-1)]
# calculate_Pre_TemBy_shape_average3(time_pairs,'mean')
# calculate_Pre_TemBy_shape_average3(time_pairs,'max')
# calculate_Pre_TemBy_shape_average3(time_pairs,'sum')
# calculate_Pre_TemBy_shape_average3(time_pairs,'effectivepre')
...
# calculate_ThresholdBy_shape_average()
